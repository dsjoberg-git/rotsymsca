# Compare results from different radome simulations.
#
# Daniel Sjöberg, 2024-07-27
...
datafolders = [('data_freestanding/', 'with taper'),
               ('data_freestanding_noantennataper/', 'no taper')]
figfolder = 'results_comparison/'
...
for theta_degrees in [0, 10, 20, 30, 40, 50]: 
    for wfactor in [10, 20]: 
        ...
        for air in [False, True]:
            for pol in ['theta', 'phi']:
                for antenna_mode in [True, False]:
                    for folder, datalabel in datafolders:
                        filename = folder + f'radome_w{wfactor}_air{air}_pol{pol}_antenna{antenna_mode}_theta{theta_degrees}_farfield.txt'
                        label = fr'pol = $\{pol}$, radome = {air^True}, {datalabel}'
                        if antenna_mode:
        ...
        plt.savefig(figfolder + f'scattering_w{wfactor}_theta{theta_degrees}.pdf')
        plt.close(fig1)
        ...
        plt.savefig(figfolder + f'transmitting_w{wfactor}_theta{theta_degrees}.pdf')
        plt.close(fig2)
